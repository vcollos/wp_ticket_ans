...
- **Kanban** `[ans_ticket_kanban]`: colunas por status (vindos de Status custom por departamento), filtros iguais ao dashboard, arrastar e soltar para alterar status, lazy load por coluna e contadores.
...
## Status
- Os status são cadastrados no wp-admin em **ANS Tickets → Status custom** como **Global** (fallback) e/ou por **Departamento**.
- Se um departamento não tiver grupo próprio completo (inicial + finais), ele usa o grupo Global.
- Quando um grupo de status do departamento é concluído após o Global, os tickets do departamento ainda no fluxo Global são migrados automaticamente para o novo fluxo respeitando as flags (inicial/meio/finais).
